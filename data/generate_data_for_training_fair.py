...
# 没有时间元素，不要后两个维度
def generate_data_and_idx2(df, x_offsets, y_offsets, add_time_of_day, add_day_of_week):
    num_samples, num_nodes = df.shape
    data = np.expand_dims(df.values, axis=-1)  # 将DataFrame（df）中的数据转换为NumPy数组，并在最后一个维度上添加一个维度
    '''只取10000个数据'''
    data = data[:10000]
...
def generate_train_val_test(args):
    ...
    data, idx = generate_data_and_idx(df, x_offsets, y_offsets, args.tod, args.dow) # 带2个时间的3维数据
    # final data shape: (105120, 716, 3) idx shape: (105097,)
    data2, idx2 = generate_data_and_idx2(df, x_offsets, y_offsets, args.tod, args.dow) # 不带时间的1维数据
    ...
    # save
    out_dir = './data/'+ args.dataset + '/' + args.years2
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    np.savez_compressed(os.path.join(out_dir, 'his.npz'), data=data, mean=scaler.mean, std=scaler.std) # 3维数据
    ...
    adj_initial_sample('./data/sd/sd_rn_adj.npy', sample_list)
...
def data_initial_sample(data):
    ...
    with open('./data/sd/sd_district2.json', 'rb') as file: # 打开 JSON 文件
        sd_district = pickle.load(file) # 读取文件内容, 字典，键(3,4)，值为list(0-715)
    sample_list = initial_sample(sd_district, args.sam_num) # 返回采样的list，其中元素为节点index(0-938)
    sample_map = sum_map(sample_list, args.sam_num) # 返回采样匹配字典，键为450个节点（0-449），值为对应的节点（0-938）
    sample_dict = sample_map_district(sample_list, sd_district) # 返回采样分区结果字典，键为区域（0-12），值为该区域对应的节点（0-938）
    ...
    np.savez_compressed(os.path.join(out_dir1, 'his_initial'+str(args.sam_num)+'.npz'), data=sample_data, sample_list=sample_list, sample_dict=sample_dict, sample_map=sample_map) #, mean=scaler.mean, std=scaler.std)
...
def adj_initial_sample(adj_path, sample_list):
    ...
    np.save(os.path.join(out_dir1, 'adj_initial'+str(args.sam_num)), new_adj) # 716
    new_adj[new_adj != 0] = 1
    np.save(os.path.join(out_dir1, 'adj_initial'+str(args.sam_num)+'_all1'), new_adj)
    print("finish!")
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--years2', type=str, default='20191w', help='if use data from multiple years, please use underline to separate them, e.g., 2018_2019')
    parser.add_argument('--years2', type=str, default='sd400', help='if use data from multiple years, please use underline to separate them, e.g., 2018_2019')
    ...
    parser.add_argument('--sam_num', type=int, default='400', help='sample sum')
