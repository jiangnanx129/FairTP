...
def get_lat_lon(data_path):
    ...
    ca_meta = pd.read_csv(data_path+"gla_meta.csv")
    ...
    print("gla的区域范围：", max_lat, min_lat, max_lon, min_lon)
...
def partition(lat_lon, max_lon, min_lon, max_lat, min_lat):
    # 计算每个区域的纬度和经度间隔
    latitude_interval = 0.5 #  (max_lat - min_lat) / x1
    longitude_interval = 1 # (max_lon - min_lon) / x1
    ...
    with open(data_path+"gla_district.json", 'wb') as file:
        pickle.dump(district_dict, file)
    with open(data_path+"gla_district_counts.json", 'wb') as file:
        pickle.dump(district_counts, file)
    with open(data_path+"gla_district_geo.json", 'wb') as file:
...
def plot_roadcount_statistics(hb_regions_counts): # data是字典
    ...
    # 调整图像布局
    plt.tight_layout()
    plt.savefig("Barchart_13_count.png", dpi=300)
    # 显示图像
    plt.show()
...
'''
1. gla: 2.1977099999999936 4.874332999999993
'''
data_path = './data/gla/'
...
with open(data_path+"gla_district_counts.json", 'rb') as file:
