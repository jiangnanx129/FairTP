...
from src.models.stgode_fair3_T4 import STGODE # 涉及到采样adj_list, baseline每T次重新采样！
from src.engines.stgode_200.stgode_engine_fair3_T3_e2 import STGODE_Engine
...
def get_config():
    ...
    parser.add_argument('--lrate', type=float, default=2e-3) # 2e-3
    ...
    logger = get_logger(log_dir, __name__, 'record_s{}_HK5_6.log'.format(args.seed))
    logger.info(args)
    logger.info("改变学习率。用的ylvalue_s2018_HK_3_2e-3_node_mape2_S_T4，学习率默认, 不重新采样")
    logger.info("用的模型：stgode_fair3_T4")
...
def main():
    ...
    model = STGODE(node_num=args.sam_num,
                ...
                #    A_sp=sp_matrix,
                #    A_se=se_matrix  # 模型调用时才用，矩阵采样在engine中做
                   )
