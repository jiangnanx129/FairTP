...
from src.engines.dcrnn_engine_forstaticfair import DCRNN_Engine
from src.utils.args import get_public_config
from src.utils.dataloader import load_dataset, load_adj_from_numpy, get_dataset_info, load_dataset2
...
def get_config():
    ...
    logger = get_logger(log_dir, __name__, 'record_s{}_HK_staticfair_2.log'.format(args.seed)) # _nonomarlize
    logger.info(args) # 打印所有参数
...
def main():
    args, log_dir, logger = get_config() # args所有预定好的参数，log_dir为文件保存地址，logger为运行文件的保存地址
    set_seed(args.seed)
    device = torch.device(args.device)

    data_path, adj_path, node_num = get_dataset_info(args.dataset) # 数据路径，邻接矩阵文件，节点数目
