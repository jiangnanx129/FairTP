...
from src.engines.d2stgnn_200.d2stgnn_engine_fair3_T3_e2_SD import D2STGNN_Engine
...
# add
    ...
    parser.add_argument('--d_lrate', type=int, default=2e-3, help='dis lr') # 鉴别器lr
    ...
    logger = get_logger(log_dir, __name__, 'record_s{}_SD5_2_noD.log'.format(args.seed))
    logger.info(args)
    logger.info("不改变学习率。用的record_s2018_SD_3_2e-3_S2_b24，学习率默认")
    logger.info("torch.optim.lr_scheduler.MultiStepLR(optimizer_D, milestones=[1, 38, 46, 54, 62, 70, 80], gamma=0.5)")
...
def main():
    ...
    # scheduler_D = torch.optim.lr_scheduler.MultiStepLR(optimizer_D, milestones=[1, 38, 46, 54, 62, 70, 80], gamma=0.5)
