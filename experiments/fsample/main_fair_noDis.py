...
from src.models.fsample_nodis import FSAMPLE # fsample4
from src.engines.fsample_engine_gx_noDis import FSAMPE_Engine # fsample_engine5
...
def get_config():
    ...
    logger = get_logger(log_dir, __name__, 'record_s{}_e6_noDis2.log'.format(args.seed)) # 日志保存位置 5,0.05,0.1
...
def main():
    ...
    ptr1 = np.load(os.path.join(data_path, args.years, 'his_initial350.npz'), allow_pickle=True) # his.npz为原始数据 (N,938,ci) ci=1
    ...
    model = FSAMPLE(node_num=sam_num,
                  scaler = scaler,
    ...
    engine = FSAMPE_Engine(criterion = criterion,
                          ...
                          sample_num = sam_num,
                          district13_road_index = sd_district,
    ...
    if args.mode == 'train':
        engine.train(sample_list, sample_map, sample_dict, district_nodes, yl_values)
    else:
