...
class FSAMPE_Engine(BaseEngine):
    def __init__(self, criterion, d_lrate, n_number,model_params, D_params, optimizer_D, scheduler_D, a_loss2, a_loss3, a_loss4, T_dynamic, sample_num, district13_road_index, **args):
    ...
    def save_model(self, save_path):
        ...
        filename = 'final_model_s{}_HK3_disout6_5.pt'.format(self._seed)
    ...
    def train_batch(self, sample_list, sample_map, sample_dict, district_nodes, yl_values, epoch, total_nodes):
        ...
        for X, label in self._dataloader['train_loader'].get_iterator(): # 循环每一个batch
            ...
            '''加上detach后，loss4没有梯度'''
            # print("66. requires_grad:", loss1.requires_grad, loss2_1.requires_grad, loss3.requires_grad, dis_out.requires_grad) # TTTT

            loss = loss1 + self.a_loss2 * loss2 + self.a_loss3 * loss3 + self.a_loss4 * loss4  # HK 0.1, dis_lobel 0.01
