...
class DCRNN_Engine(BaseEngine):
    ...
    def save_model(self, save_path):
        ...
        filename = 'final_model_s{}_HK_forstaticfair_2.pt'.format(self._seed)
    ...
    def train_batch(self, district_nodes, epoch):
        self.model.train() # model start train

        train_loss = []
        train_mape = []
        train_mape2 = []
        train_rmse = []
        static_fair, dynamic_fair = [],[0]
        batch_count = 0
        ...
        for X, label in self._dataloader['train_loader'].get_iterator():
            ...
            loss_message = 'Epoch: {:03d}, Batch_num:{:03d}, mape2:{:.4f}, Loss1: {:.4f}, Loss3: {:.4f}'
            self._logger.info(loss_message.format(epoch + 1, batch_count+1, mape2, loss, loss3))
    ...
    def train(self, district_nodes):
        ...
        for epoch in range(self._max_epochs):
            t1 = time.time()
            mtrain_loss, mtrain_mape, mtrain_mape2, mtrain_rmse, mtrain_sfair, mtrain_dfair = self.train_batch(district_nodes, epoch)
            t2 = time.time()

            v1 = time.time()
            mvalid_loss, mvalid_mape, mvalid_mape2, mvalid_rmse, mvalid_sfair, mvalid_dfair = self.evaluate('val', district_nodes, epoch)
            v2 = time.time()
    ...
    def evaluate(self, mode, district_nodes, epoch):
        ...
        with torch.no_grad():
            for X, label in self._dataloader[mode + '_loader'].get_iterator():
                ...
                # for introduction original static fairness
                ...
                loss_message = 'Epoch: {:03d}, Batch_num:{:03d}, mape2: {:.4f}, Loss1: {:.4f}, Loss3: {:.4f}, region_mae:{}'
                self._logger.info(loss_message.format(epoch + 1, batch_count+1, mape2, loss, loss3, loss_region))
        ...
        if mode == 'val':
            ...
            # 保存列表到文件
            with open('HK_list_pred_dcrnn_forstaticfair_2.pkl', 'wb') as f:
                pickle.dump(new_pred_list, f)
            with open('HK_list_label_dcrnn_forstaticfair_2.pkl', 'wb') as f:
