...
class FSAMPE_Engine(BaseEngine):
    ...
    def save_model(self, save_path):
        ...
        filename = 'final_model_s{}_HK3_disout7_8h.pt'.format(self._seed)
    ...
    def train_batch(self, sample_list, sample_map, sample_dict, district_nodes, yl_values, epoch, total_nodes):
        ...
        for X, label in self._dataloader['train_loader'].get_iterator(): # 循环每一个batch
            ...
            loss4,  loss5, greater_than_zero, less_than_zero, equal_to_zero,\
             gmax_value, gmin_value, gav, gerror,  lmax_value, lmin_value, lav, lerror \
                = dynamic_cal5_global(dis_labels, dis_out, sample_map, district_nodes) # , self.district13_road_index, node_count_global) # 更新了node_count_global
            ...
            2. loss2：鉴别器损失
            '''
            loss = loss1 + self.a_loss3*loss3 + self.a_loss4*loss4 + 0.001*loss5  # HK 0.1, dis_lobel 0.01
            ...
            yl_discrepancy.append(loss5.item())
            ...
            loss_message = 'Epoch: {:03d}, Batch_num:{:03d}, Loss1: {:.4f}, Dis Loss: {:.4f}, Loss3: {:.4f}, Loss4: {:.4f}, Loss5: {:.4f}, Benefit: {:.4f}, Sacrifice: {:.4f}, Equal: {:.4f}, Be: {:.4f}, Sa: {:.4f}, Eq: {:.4f},  Bmax: {:.4f}, Bmin: {:.4f},Bmea: {:.4f}, Ber: {:.4f}, Smax: {:.4f}, Smin: {:.4f}, Smea: {:.4f},Ser: {:.4f}'  # , Loss44: {:.4f}
            self._logger.info(loss_message.format(epoch + 1, batch_count+1, loss1, loss2, loss3, loss4, loss5, greater_than_zero, less_than_zero, equal_to_zero, benum, sanum,eqnum,  gmax_value, gmin_value, gav,gerror,  lmax_value, lmin_value,lav, lerror)) # , loss44))
        ...
        return np.mean(train_loss), np.mean(train_mape), np.mean(train_rmse), np.mean(dis_loss), np.mean(static_fair), np.mean(dynamic_fair), np.mean(yl_discrepancy) \
    ...
    def train(self, sample_list, sample_map, sample_dict, district_nodes, yl_values):
        ...
        for epoch in range(self._max_epochs):
            t1 = time.time()
            mtrain_loss, mtrain_mape, mtrain_rmse, mtraind_loss, mtrain_sfair, mtrain_dfair, mtrain_yldis, mtrain_be, mtrain_sa, mtrain_eq, mtrain_ben, mtrain_san, mtrain_eqn \
            ...
            mvalid_loss, mvalid_mape, mvalid_rmse, mvalidd_loss, mvalid_sfair, mvalid_dfair, mvalid_yldis, mvalid_be, mvalid_sa, mvalid_eq, mvalid_ben, mvalid_san, mvalid_eqn\
            ...
            message = 'Epoch: {:03d}, Train, Loss: {:.4f}, RMSE: {:.4f}, MAPE: {:.4f}, dis_loss: {:.4f}, SFair: {:.4f}, DFair: {:.4f}, YL_Dis: {:.4f},  Be {:.4f}, Sa {:.4f}, Eq {:.4f}, Bnum {:.4f}, Snum {:.4f},Enum {:.4f},Bmean {:.4f}, Smean {:.4f}, Time: {:.4f}s/epoch, LR: {:.4e}, D_LR: {:.4e}'
            self._logger.info(message.format(epoch + 1, mtrain_loss, mtrain_rmse, mtrain_mape, mtraind_loss, mtrain_sfair, mtrain_dfair, mtrain_yldis, \
                                             mtrain_be, mtrain_sa, mtrain_eq, mtrain_ben, mtrain_san, mtrain_eqn, mtrain_bmean, mtrain_smean,\
                                             (t2 - t1), cur_lr, cur_lr_d))
            message = 'Epoch: {:03d}, Test,  Loss: {:.4f}, RMSE: {:.4f}, MAPE: {:.4f}, dis_loss: {:.4f}, SFair: {:.4f}, DFair: {:.4f}, YL_Dis: {:.4f},  Be {:.4f}, Sa {:.4f}, Eq {:.4f}, Bnum {:.4f}, Snum {:.4f},Enum {:.4f},Bmean {:.4f}, Smean {:.4f}, Time: {:.4f}s, LR: {:.4e}, D_LR: {:.4e}'
            self._logger.info(message.format(epoch + 1, mvalid_loss, mvalid_rmse, mvalid_mape, mvalidd_loss, mvalid_sfair, mvalid_dfair, mvalid_yldis,\
    ...
    def evaluate(self, mode, sample_list, sample_map, sample_dict, district_nodes, yl_values,epoch,total_nodes):
        ...
        with torch.no_grad():
            for X, label in self._dataloader[mode + '_loader'].get_iterator():
                ...
                loss_message = 'Epoch: {:03d}, Batch_num:{:03d}, Loss1: {:.4f}, Dis Loss: {:.4f}, Loss3: {:.4f}, Loss4: {:.4f},Loss5: {:.4f}, Benefit: {:.4f}, Sacrifice: {:.4f}, Equal: {:.4f}, Be: {:.4f}, Sa: {:.4f}, Eq: {:.4f},  Bmax: {:.4f}, Bmin: {:.4f},Bmea: {:.4f}, Ber: {:.4f}, Smax: {:.4f}, Smin: {:.4f}, Smea: {:.4f},Ser: {:.4f}'  # , Loss44: {:.4f}
                self._logger.info(loss_message.format(epoch + 1, batch_count+1, loss1, loss2, loss3, loss4,loss5, greater_than_zero, less_than_zero, equal_to_zero, ebenum, esanum,eeqnum,  gmax_value, gmin_value, gav,gerror,  lmax_value, lmin_value,lav, lerror)) # , loss44))
                ...
                edynamic_fair.append(loss4.item())
                eyl_discrepancy.append(loss5.item())
        ...
        if mode == 'val':
            ...
            mvalid_loss, mvalid_mape, mvalid_rmse, mvalidd_loss, mvalid_sfair, mvalid_dfair,mvalid_yldis, mvalid_be, mvalid_sa, mvalid_eq, mvalid_ben, mvalid_san, mvalid_eqn\
            , mvalid_bmean, mvalid_smean\
                  = np.mean(e_loss), np.mean(e_mape), np.mean(e_rmse), np.mean(edis_loss), np.mean(estatic_fair), np.mean(edynamic_fair), np.mean(eyl_discrepancy)\
            ...
            return mvalid_loss, mvalid_mape, mvalid_rmse, mvalidd_loss, mvalid_sfair, mvalid_dfair, mvalid_yldis , mvalid_be, mvalid_sa, mvalid_eq, mvalid_ben, mvalid_san, mvalid_eqn, mvalid_bmean, mvalid_smean
            ...
            log = 'Average Test MAE: {:.4f}, RMSE: {:.4f}, MAPE: {:.4f}, D_Loss: {:.4f}, SFair: {:.4f}, DFair: {:.4f}, YL_Dis: {:.4f}, Be {:.4f}, Sa {:.4f}, Eq {:.4f}, Bnum {:.4f}, Snum {:.4f},Enum {:.4f},Bmean {:.4f}, Smean {:.4f},.'
            self._logger.info(log.format(np.mean(e_loss), np.mean(e_rmse), np.mean(e_mape), np.mean(edis_loss), np.mean(estatic_fair), np.mean(edynamic_fair), np.mean(eyl_discrepancy)\
                                         , np.mean(ebenefit_list), np.mean(esarcifice_list), np.mean(eequal_list) \
            , np.mean(ebenefit_list)/(np.mean(ebenefit_list)+np.mean(esarcifice_list)+np.mean(eequal_list)) \
            , np.mean(esarcifice_list)/(np.mean(ebenefit_list)+np.mean(esarcifice_list)+np.mean(eequal_list)) \
            ...
            # pred, dis_labels, yl_label, dis_out, mape_data = self.model(X, label, sample_list, yl_values, self._iter_cnt)
            # print("55. requires_grad:", pred.requires_grad, dis_labels.requires_grad, yl_label.requires_grad, dis_out.requires_grad)
            ...
            # loss4, node_count_global = dynamic_cal5_global(dis_labels, dis_out, sample_map, district_nodes, self.district13_road_index, node_count_global) # 更新了node_count_global
            # print("66. requires_grad:", loss1.requires_grad, loss2.requires_grad, loss3.requires_grad, loss4.requires_grad)
            ...
            # 2. 鉴别器损失
            # '''
            # loss = loss1 + 0.01*loss3 + 0.01*loss4  # HK 0.1, dis_lobel 0.01
            # # loss = loss1 + 0.05*loss2 + 0.01*loss3 + 0.02*loss4 # HK2， o做输入。
