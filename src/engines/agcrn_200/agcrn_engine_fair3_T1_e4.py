...
class AGCRN_Engine(BaseEngine):
    ...
    def save_model(self, save_path):
        ...
        filename = 'final_model_s{}_HK3_disout7_12_e2.pt'.format(self._seed)
    ...
    def train_batch(self, time_T, sample_list, sample_map, sample_dict, district_nodes, yl_values, epoch, total_nodes):
        ...
        for X, label in self._dataloader['train_loader'].get_iterator():
            ...
            # self.model_params.requires_grad_(True) 
            # self.D_params.requires_grad_(False)
            ...
            pred1, label1 = self._inverse_transform([pred, label])
            ...
            for region, nodes_list in district_nodes.items(): # region是str '0' # sample2.py中所有round+0.5确保0-12均有采样，即有0-12
                new_pred[:, :, int(region)] = pred1[:, :, nodes_list].mean(dim=2)#, keepdim=True)#.mean(dim=2)
            ...
            if batch_count>=(time_T-1):#(batch_count+1)% time_T == 0:
            ...
            # self.model_params.requires_grad_(False) 
            # self.D_params.requires_grad_(True)
            for param in self.model_params:
                param.requires_grad_(False)
            for param in self.D_params:
                param.requires_grad_(True)
