...
'''
同fsample_engine3.py
不同：改变采样方式，采样用到静态和动态的反馈
采样方法改变，采样优势节点dynamic_cal5_2，采样误差小的区域static_cal_2
'''
...
class FSAMPE_Engine(BaseEngine):
    ...
    def save_model(self, save_path):
        ...
        filename = 'final_model_s{}_HK3_disout11_2.pt'.format(self._seed)
    ...
    def train_batch(self, sample_list, sample_map, sample_dict, district_nodes, yl_values, epoch, total_nodes):
        ...
        for X, label in self._dataloader['train_loader'].get_iterator(): # 循环每一个batch
            ...
            loss3, values_region = static_cal_2(new_pred, new_label, self._device) # 静态公平正则化
            # values_region归一化后的tensor，13个；values_node为归一化后的tensor，938
            loss4, values_node, greater_than_zero, less_than_zero, equal_to_zero = dynamic_cal5_2(dis_labels, dis_out, sample_map, district_nodes, self.district13_road_index, self._device)
            # loss4 = dynamic_cal5_global(dis_labels, dis_out, sample_map, district_nodes) # , self.district13_road_index, node_count_global) # 更新了node_count_global
            ...
            self._logger.info(loss_message.format(epoch + 1, batch_count+1, loss1, loss2, loss3, loss4,greater_than_zero, less_than_zero, equal_to_zero, benum, sanum,eqnum)) # , loss44))

            '''重新采样部分, 考虑动态损失+静态损失的反馈'''
            # sample_list, node_count_global = optimize_selection(self.district13_road_index, self.sample_num, node_count_global)
            sample_list, node_count_global = optimize_selection_withfair(values_region, values_node, self.district13_road_index, self.sample_num, node_count_global, self._device)
    ...
    def train(self, sample_list, sample_map, sample_dict, district_nodes, yl_values):
        ...
        for epoch in range(self._max_epochs):
            ...
            message = 'Epoch: {:03d}, Train, Loss: {:.4f}, RMSE: {:.4f}, MAPE: {:.4f}, dis_loss: {:.4f}, SFair: {:.4f}, DFair: {:.4f}, Be {:.4f}, Sa {:.4f}, Eq {:.4f}, Bnum {:.4f}, Snum {:.4f},Enum {:.4f}, Time: {:.4f}s/epoch, LR: {:.4e}, D_LR: {:.4e}'
            ...
            message = 'Epoch: {:03d}, Test,  Loss: {:.4f}, RMSE: {:.4f}, MAPE: {:.4f}, dis_loss: {:.4f}, SFair: {:.4f}, DFair: {:.4f}, Be {:.4f}, Sa {:.4f}, Eq {:.4f}, Bnum {:.4f}, Snum {:.4f},Enum {:.4f}, Time: {:.4f}s, LR: {:.4e}, D_LR: {:.4e}'
    ...
    def evaluate(self, mode, sample_list, sample_map, sample_dict, district_nodes, yl_values,epoch,total_nodes):
        ...
        if mode == 'val':
            ...
            return mvalid_loss, mvalid_mape, mvalid_rmse, mvalidd_loss, mvalid_sfair, mvalid_dfair, mvalid_be, mvalid_sa, mvalid_eq, mvalid_ben, mvalid_san, mvalid_eqn
            ...
            log = 'Average Test MAE: {:.4f}, RMSE: {:.4f}, MAPE: {:.4f}, D_Loss: {:.4f}, SFair: {:.4f}, DFair: {:.4f}, Be {:.4f}, Sa {:.4f}, Eq {:.4f}, Bnum {:.4f}, Snum {:.4f},Enum {:.4f}'
