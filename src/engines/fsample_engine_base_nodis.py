...
import random
from src.engines.sample_base_nodis import *
'''0924创建，没有鉴别器'''
...
class FSAMPE_Engine(BaseEngine):
    ...
    def save_model(self, save_path):
        ...
        filename = 'final_model_s{}_HK_0924_1.pt'.format(self._seed)
    ...
    def train_batch(self, sample_list, sample_map, sample_dict, district_nodes, yl_values, epoch):
        ...
        for X, label in self._dataloader['train_loader'].get_iterator(): # 循环每一个batch
            ...
            '''没有鉴别器，所以没有loss2的分类损失'''
            loss3_mpe, loss3_mape, values_region_mpe, values_region_mape = static_cal(new_pred, new_label) # 静态公平正则化
            pirnt()
