...
class AGCRN_Engine(BaseEngine):
    ...
    def save_model(self, save_path):
        ...
        filename = 'final_model_s{}_HK3_disout7_6.pt'.format(self._seed)
    ...
    def train_batch(self, time_T, sample_list, sample_map, sample_dict, district_nodes, yl_values, epoch, total_nodes):
        ...
        for X, label in self._dataloader['train_loader'].get_iterator():
            ...
            if (batch_count+1) % time_T == 0:
                time_T_cal = 1
                loss4, greater_than_zero, less_than_zero, equal_to_zero, node_yl_dic_disout \
                = dynamic_cal5_global_no5_3(time_T_cal, dis_labels, dis_out, sample_map, district_nodes, node_yl_dic_disout) # , self.district13_road_index, node_count_global) # 更新了node_count_global
                node_yl_dic_disout.clear() # 清空字典

                loss = loss1 + self.a_loss3*loss3 + self.a_loss4*loss4  # HK 0.1, dis_lobel 0.01
                dynamic_fair.append(loss4.item()) # 个体动态损失
            else:
                time_T_cal = 0
                greater_than_zero, less_than_zero, equal_to_zero, node_yl_dic_disout \
                = dynamic_cal5_global_no5_3(time_T_cal, dis_labels, dis_out, sample_map, district_nodes, node_yl_dic_disout) 
            ...
            if (batch_count+1) % time_T == 0:
                ...
                loss_message = 'Epoch: {:03d}, Batch_num:{:03d}, mape2:{:.4f}, Loss1: {:.4f}, Dis Loss: {:.4f}, Loss3: {:.4f},  Benefit: {:.4f}, Sacrifice: {:.4f}, Equal: {:.4f}'  # , Loss44: {:.4f}
                self._logger.info(loss_message.format(epoch + 1, batch_count+1, mape2, loss1, loss2, loss3,  greater_than_zero, less_than_zero, equal_to_zero)) # , loss44))


            '''
            重新采样部分：1.每个batch根据12原则，2. 每个batch根据123，3.每个batch根据123，每T个batch根据1234
            '''
            print("a:",len(list(node_yl_dic_disout.keys()))) # , list(node_yl_dic_disout.keys())
            print("b:",len(list(node_count_global.keys()))) # , list(node_count_global.keys())
    ...
    def train(self, time_T, sample_list, sample_map, sample_dict, district_nodes, yl_values):
        ...
        self._logger.info(yl_values)
        self.evaluate('test',time_T, sample_list, sample_map, sample_dict, district_nodes, yl_values, epoch, total_nodes, node_yl_dic_disout)




    def evaluate(self, mode, time_T, sample_list, sample_map, sample_dict, district_nodes, yl_values,epoch,total_nodes, node_yl_dic_disout):
        ...
        with torch.no_grad():
            for X, label in self._dataloader[mode + '_loader'].get_iterator():
                ...
                if (batch_count+1) % time_T == 0:
                    ...
                    # loss4, greater_than_zero, less_than_zero, equal_to_zero = dynamic_cal5_global_no5(dis_labels, dis_out, sample_map, district_nodes) # , self.district13_road_index, node_count_global) # 更新了node_count_global
                    loss4, greater_than_zero, less_than_zero, equal_to_zero, node_yl_dic_disout\
                        = dynamic_cal5_global_no5_3(dis_labels, dis_out, sample_map, district_nodes, node_yl_dic_disout) # , self.district13_road_index, node_count_global) # 更新了node_count_global
