...
class AGCRN_Engine(BaseEngine):
    def __init__(self, T_dynamic, sample_num, district13_road_index, **args):
    ...
    def save_model(self, save_path):
        ...
        filename = 'final_model_s{}_HK_3_1e-2_S_all.pt'.format(self._seed)
    ...
    def train(self, sample_list, time_T, yl_values, sample_map, district_nodes):
        ...
        # 将两个列表存储到文件中
        filename1 = 'ylvalue_s{}_HK_3_1e-2_node_mpe2_S_all.pkl'.format(self._seed)
        with open(os.path.join(self._save_path, filename1), 'wb') as f:
            pickle.dump((yl_list_train, yl_list_val), f)

        filename2 = 'ylvalue_s{}_HK_3_1e-2_node_mae2_S_all.pkl'.format(self._seed)
        with open(os.path.join(self._save_path, filename2), 'wb') as f:
            pickle.dump((yl_list_train_node, yl_list_val_node), f)

        filename1 = 'ylvalue_s{}_HK_3_1e-2_node_mape2_S_all.pkl'.format(self._seed)
        with open(os.path.join(self._save_path, filename1), 'wb') as f:
    ...
    def evaluate(self, mode, sample_list, time_T, yl_values, sample_map, district_nodes, epoch):
        ...
        with torch.no_grad():
            for X, label in self._dataloader[mode + '_loader'].get_iterator():
                ...
                # loss_region_mpe = masked_mpe2(new_pred, new_label, mask_value)
                loss_region_mpe = masked_mpe3(new_pred, new_label)
                loss_region_mape = masked_mape33(new_pred, new_label)
                ...
                if mode == "test":
                    ...
                    mpe_region_list.append(loss_region_mpe)
                    mape_region_list.append(loss_region_mape)
