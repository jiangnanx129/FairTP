...
class FSAMPE_Engine(BaseEngine):
    ...
    def save_model(self, save_path):
        ...
        filename = 'final_model_s{}_HK3_disout12_1.pt'.format(self._seed)
    ...
    def train_batch(self, sample_list, sample_map, sample_dict, district_nodes, yl_values, epoch, total_nodes):
        ...
        for X, label in self._dataloader['train_loader'].get_iterator(): # 循环每一个batch
            ...
            self._logger.info(loss_message.format(epoch + 1, batch_count+1, loss1, loss2, loss3, loss4,greater_than_zero, less_than_zero, equal_to_zero, benum, sanum,eqnum,  gmax_value, gmin_value, gav,gerror,  lmax_value, lmin_value,lav, lerror)) # , loss44))
            ...
            '''重新采样部分'''
            sample_list, node_count_global = optimize_selection_equal(self.district13_road_index, self.sample_num, node_count_global, self._device)
