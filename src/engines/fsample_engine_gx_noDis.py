...
class FSAMPE_Engine(BaseEngine):
    ...
    def save_model(self, save_path):
        ...
        filename = 'final_model_s{}_SDnoDis.pt'.format(self._seed)
    ...
    def train_batch(self, sample_list, sample_map, sample_dict, district_nodes, yl_values, epoch):
        ...
        for X, label in self._dataloader['train_loader'].get_iterator(): # 循环每一个batch
            ...
            pred = self.model(X, label, sample_list, yl_values, self._iter_cnt)
            ...
            loss = loss1 + 0.05*loss3  # test 40, 0.1,0.15
            loss_message = 'Epoch: {:03d}, Batch_num:{:03d}, Loss1: {:.4f}, Loss3: {:.4f}'
            self._logger.info(loss_message.format(epoch + 1, batch_count+1, loss1, loss3))
            ...
            sorted_dict = pro_sample4(values_region, self.district13_road_index)
        ...
        return np.mean(train_loss), np.mean(train_mape), np.mean(train_rmse), np.mean(static_fair) # 一个epoch内！多个batch所以mean
    ...
    def train(self, sample_list, sample_map, sample_dict, district_nodes, yl_values):
        ...
        for epoch in range(self._max_epochs):
            t1 = time.time()
            mtrain_loss, mtrain_mape, mtrain_rmse, mtrain_sfair = self.train_batch(sample_list, sample_map, sample_dict, district_nodes, yl_values, epoch)
            t2 = time.time()

            v1 = time.time()
            mvalid_loss, mvalid_mape, mvalid_rmse, mvalid_sfair = self.evaluate('val',sample_list, sample_map, sample_dict, district_nodes, yl_values, epoch)
            v2 = time.time()
            ...
            message = 'Epoch: {:03d}, Train Loss: {:.4f}, Train RMSE: {:.4f}, Train MAPE: {:.4f}, Train SFair: {:.4f}, Valid Loss: {:.4f}, Valid RMSE: {:.4f}, Valid MAPE: {:.4f}, Valid SFair: {:.4f}, Train Time: {:.4f}s/epoch, Valid Time: {:.4f}s, LR: {:.4e}'
            self._logger.info(message.format(epoch + 1, mtrain_loss, mtrain_rmse, mtrain_mape, mtrain_sfair, \
                                             mvalid_loss, mvalid_rmse, mvalid_mape, mvalid_sfair, \
                                             (t2 - t1), (v2 - v1), cur_lr))
    ...
    def evaluate(self, mode, sample_list, sample_map, sample_dict, district_nodes, yl_values,epoch):
        ...
        if mode == 'val':
            ...
            mvalid_loss, mvalid_mape, mvalid_rmse, mvalid_sfair = \
                np.mean(e_loss), np.mean(e_mape), np.mean(e_rmse), np.mean(estatic_fair)
                
            return mvalid_loss, mvalid_mape, mvalid_rmse, mvalid_sfair
            ...
            self._logger.info(log.format(np.mean(e_loss), np.mean(e_rmse), np.mean(e_mape), np.mean(estatic_fair)))
