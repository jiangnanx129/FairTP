...
#     ptr = np.load(os.path.join(data_path, args.years, 'his_initial200.npz')) # hk450
#     data = ptr['data'][..., 0]
#     sample_num, node_num = data.shape # 无妨，总共多少条数据，节点数目
...
# def get_sample_adj(sample_list, adj_mx, data_path, args):

#     new_adj = adj_mx[sample_list] # 8k*8k, select 716*8k
#     new_adj = new_adj[:,sample_list]

#     new_adj = new_adj - np.eye(args.sam_num) # 采样的数目

#     sp_matrix = new_adj + np.transpose(new_adj)
#     sp_matrix = normalize_adj_mx(sp_matrix).to(args.device)
    
#     se_matrix = construct_se_matrix(data_path, args) # 要读取HK数据的npz文件， initial450
#     se_matrix = normalize_adj_mx(se_matrix).to(args.device)

#     return sp_matrix, se_matrix



class STGODE_Engine(BaseEngine):
    def __init__(self, data_path, adj_mx, T_dynamic, sample_num, district13_road_index, \
                 years, tpd, sigma, thres, **args):
        super(STGODE_Engine, self).__init__(**args)
    ...
    def save_model(self, save_path):
        ...
        filename = 'final_model_s{}_HK_3_2e-3_S_T4.pt'.format(self._seed)
    ...
    def construct_se_matrix(self):
        ptr = np.load(os.path.join(self.data_path, self.years, 'his_initial200.npz')) # hk450
        data = ptr['data'][..., 0]
        sample_num, node_num = data.shape # 无妨，总共多少条数据，节点数目

        data_mean = np.mean([data[self.tpd * i: self.tpd * (i + 1)] for i in range(sample_num // self.tpd)], axis=0)
        data_mean = data_mean.T
        ...
        std = np.std(dist_matrix)
        dist_matrix = (dist_matrix - mean) / std
        dist_matrix = np.exp(-dist_matrix ** 2 / self.sigma ** 2)
        dtw_matrix = np.zeros_like(dist_matrix)
        dtw_matrix[dist_matrix > self.thres] = 1
        return dtw_matrix
    ...
    def get_sample_adj(self, sample_list):

        new_adj = self.adj_mx[sample_list] # 8k*8k, select 716*8k
        new_adj = new_adj[:,sample_list]

        new_adj = new_adj - np.eye(self.sample_num) # 采样的数目

        sp_matrix = new_adj + np.transpose(new_adj)
        sp_matrix = normalize_adj_mx(sp_matrix).to(self._device)
    
        se_matrix = self.construct_se_matrix() # 要读取HK数据的npz文件， initial450
        se_matrix = normalize_adj_mx(se_matrix).to(self._device)
    ...
    def train_batch(self, sample_list, time_T, yl_values, sample_map, district_nodes, epoch):
        ...
        for X, label in self._dataloader['train_loader'].get_iterator():
            ...
            sp_matrix, se_matrix = self.get_sample_adj(sample_list)
            # print(sp_matrix.shape, se_matrix.shape)
            pred, dis_out = self.model(X, sp_matrix, se_matrix, label) # 输出dis_out但在baseline中不用
    ...
    def train(self, sample_list, time_T, yl_values, sample_map, district_nodes):
        ...
        # 将两个列表存储到文件中
        filename1 = 'ylvalue_s{}_HK_3_2e-3_node_mpe2_S_T4.pkl'.format(self._seed)
        with open(os.path.join(self._save_path, filename1), 'wb') as f:
            pickle.dump((yl_list_train, yl_list_val), f)

        filename2 = 'ylvalue_s{}_HK_3_2e-3_node_mae2_S_T4.pkl'.format(self._seed)
        with open(os.path.join(self._save_path, filename2), 'wb') as f:
            pickle.dump((yl_list_train_node, yl_list_val_node), f)

        # 用这个！计算的是mape！
        filename1 = 'ylvalue_s{}_HK_3_2e-3_node_mape2_S_T4.pkl'.format(self._seed)
        with open(os.path.join(self._save_path, filename1), 'wb') as f:
