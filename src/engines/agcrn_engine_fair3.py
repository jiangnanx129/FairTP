...
class AGCRN_Engine(BaseEngine):
    ...
    def save_model(self, save_path):
        ...
        filename = 'final_model_s{}_HK3_disout7_2.pt'.format(self._seed)
    ...
    def train_batch(self, sample_list, sample_map, sample_dict, district_nodes, yl_values, epoch, total_nodes):
        ...
        for X, label in self._dataloader['train_loader'].get_iterator():
            ...
            self._logger.info(loss_message.format(epoch + 1, batch_count+1, mape2, loss1, loss2_2, loss3, loss4,  greater_than_zero, less_than_zero, equal_to_zero, benum, sanum,eqnum,  gmax_value, gmin_value, gav,gerror,  lmax_value, lmin_value,lav, lerror)) # , loss44))
    ...
    def evaluate(self, mode, sample_list, sample_map, sample_dict, district_nodes, yl_values,epoch,total_nodes):
        ...
        with torch.no_grad():
            for X, label in self._dataloader[mode + '_loader'].get_iterator():
                ...
                loss_message = 'Epoch: {:03d}, Batch_num:{:03d}, mape2:{:.4f}, Loss1: {:.4f}, Dis Loss: {:.4f}, Loss3: {:.4f}, Loss4: {:.4f}, Benefit: {:.4f}, Sacrifice: {:.4f}, Equal: {:.4f}, Be: {:.4f}, Sa: {:.4f}, Eq: {:.4f},  Bmax: {:.4f}, Bmin: {:.4f},Bmea: {:.4f}, Ber: {:.4f}, Smax: {:.4f}, Smin: {:.4f}, Smea: {:.4f},Ser: {:.4f}'  # , Loss44: {:.4f}
                self._logger.info(loss_message.format(epoch + 1, batch_count+1, mape2, loss1, loss2, loss3, loss4, greater_than_zero, less_than_zero, equal_to_zero, ebenum, esanum,eeqnum,  gmax_value, gmin_value, gav,gerror,  lmax_value, lmin_value,lav, lerror)) # , loss44))
