...
分开了encoder，decoder || 和discriminator
'''

class FSAMPE_Engine(BaseEngine):
    def __init__(self, criterion, d_lrate, n_number, optimizer_D, scheduler_D, T_dynamic, sample_num, district13_road_index, **args):
    ...
    def save_model(self, save_path):
        ...
        filename = 'final_model_s{}_HK3_dislabel2.pt'.format(self._seed)
    ...
    def train_batch(self, sample_list, sample_map, sample_dict, district_nodes, yl_values, epoch, total_nodes):
        ...
        for X, label in self._dataloader['train_loader'].get_iterator(): # 循环每一个batch
            ...
            2. 鉴别器损失
            '''
            loss = loss1 + 0.01*loss3 + 0.005*loss4  # HK 0.1, dis_lobel 0.01
            ...
            '''训练鉴别器, 控制encoder和decoder，loss2就不能梯度下降'''
            # self.model.encoder.requires_grad_(False) 
            # self.model.decoder.requires_grad_(False)
            self.optimizer_D.zero_grad() # 鉴别器优化器
            pred, dis_labels, yl_label, dis_out, mape_data = self.model(X, label, sample_list, yl_values, self._iter_cnt)
            loss2 = self.criterion(dis_out.reshape(b,t,n,-1), yl_label.float()) # 鉴别器损失，450个节点的准确优劣分类
            # print("22. loss2.requires_grad:", loss2.requires_grad)
            loss2.backward()
            if self._clip_grad_value != 0:
                torch.nn.utils.clip_grad_norm_(self.model.classif.parameters(), self._clip_grad_value)
