...
def get_sample_adj_list(sample_list, adj_mx, sam_num, order, device):
        
    new_adj = adj_mx[sample_list] # 8k*8k, select 716*8k
    new_adj = new_adj[:,sample_list]
    new_adj = new_adj - np.eye(sam_num)

    adj = np.zeros((sam_num, sam_num), dtype=np.float32)
    for n in range(sam_num):
        idx = np.nonzero(new_adj[n])[0]
        adj[n, idx] = 1

    L_tilde = normalize_adj_mx(adj, 'scalap')[0]
    cheb_poly = [torch.from_numpy(i).type(torch.FloatTensor).to(device) for i in calculate_cheb_poly(L_tilde, order)] 
    adj = torch.tensor(adj).to(device)

    return cheb_poly, adj


class DSTAGNN_Engine(BaseEngine):
    def __init__(self, adj_mx, order, sam_num, T_dynamic, sample_num, district13_road_index, **args):
    ...
    def save_model(self, save_path):
        ...
        filename = 'final_model_s{}_SD_3_1e-4_S_b48.pt'.format(self._seed)
    ...
    def train_batch(self, sample_list, time_T, yl_values, sample_map, district_nodes, epoch):
        ...
        for X, label in self._dataloader['train_loader'].get_iterator():
            ...
            cheb_poly, adj = get_sample_adj_list(sample_list, self.adj_mx, self.sam_num, self.order,self._device)
            pred, dis_out = self.model(X, cheb_poly, adj, label) # 输出dis_out但在baseline中不用
    ...
    def train(self, sample_list, time_T, yl_values, sample_map, district_nodes):
        ...
        # 将两个列表存储到文件中
        filename1 = 'ylvalue_s{}_SD_3_1e-4_node_mpe2_S_b48.pkl'.format(self._seed)
        with open(os.path.join(self._save_path, filename1), 'wb') as f:
            pickle.dump((yl_list_train, yl_list_val), f)

        filename2 = 'ylvalue_s{}_SD_3_1e-4_node_mae2_S_b48.pkl'.format(self._seed)
        with open(os.path.join(self._save_path, filename2), 'wb') as f:
            pickle.dump((yl_list_train_node, yl_list_val_node), f)

        # 用这个！计算的是mape！
        filename1 = 'ylvalue_s{}_SD_3_1e-4_node_mape2_S_b48.pkl'.format(self._seed)
        with open(os.path.join(self._save_path, filename1), 'wb') as f:
