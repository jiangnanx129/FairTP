...
效率很低，mape数据根据当前batch中的平均值，每个batch都不一样
'''
...
class FSAMPE_Engine(BaseEngine):
    ...
    def save_model(self, save_path):
        ...
        filename = 'final_model_s{}_HK3_disout7_8i6.pt'.format(self._seed)
    ...
    def train_batch(self, sample_list, sample_map, sample_dict, district_nodes, yl_values, epoch, total_nodes):
        ...
        for X, label in self._dataloader['train_loader'].get_iterator(): # 循环每一个batch
            ...
            loss_message = 'Epoch: {:03d}, Batch_num:{:03d}, mape2:{:.4f}, Loss1: {:.4f}, Dis Loss: {:.4f}, Loss3: {:.4f}, Loss4: {:.4f},  Benefit: {:.4f}, Sacrifice: {:.4f}, Equal: {:.4f}, Be: {:.4f}, Sa: {:.4f}, Eq: {:.4f}'  # , Loss44: {:.4f}
            self._logger.info(loss_message.format(epoch + 1, batch_count+1, mape2, loss1, loss2, loss3, loss4,  greater_than_zero, less_than_zero, equal_to_zero, benum, sanum,eqnum)) # , loss44))
        ...
        return np.mean(train_loss), np.mean(train_mape), np.mean(train_mape2), np.mean(train_rmse), np.mean(dis_loss), np.mean(static_fair), np.mean(dynamic_fair) \
            ...
            , np.mean(equal_list)/(np.mean(benefit_list)+np.mean(sarcifice_list)+np.mean(equal_list)) # 一个epoch内！多个batch所以mean
    ...
    def train(self, sample_list, sample_map, sample_dict, district_nodes, yl_values):
        ...
        for epoch in range(self._max_epochs):
            ...
            message = 'Epoch: {:03d}, Train, mape2: {:.4f}, Loss: {:.4f}, RMSE: {:.4f}, MAPE: {:.4f}, dis_loss: {:.4f}, SFair: {:.4f}, DFair: {:.4f}, Be {:.4f}, Sa {:.4f}, Eq {:.4f}, Bnum {:.4f}, Snum {:.4f},Enum {:.4f}, Time: {:.4f}s/epoch, LR: {:.4e}, D_LR: {:.4e}'
            self._logger.info(message.format(epoch + 1, mtrain_mape2, mtrain_loss, mtrain_rmse, mtrain_mape, mtraind_loss, mtrain_sfair, mtrain_dfair, \
                                             mtrain_be, mtrain_sa, mtrain_eq, mtrain_ben, mtrain_san, mtrain_eqn, \
                                             (t2 - t1), cur_lr, cur_lr_d))
            message = 'Epoch: {:03d}, Test,  mape2: {:.4f}, Loss: {:.4f}, RMSE: {:.4f}, MAPE: {:.4f}, dis_loss: {:.4f}, SFair: {:.4f}, DFair: {:.4f},  Be {:.4f}, Sa {:.4f}, Eq {:.4f}, Bnum {:.4f}, Snum {:.4f},Enum {:.4f}, Time:{:.4f}s/epoch, LR: {:.4e}, D_LR: {:.4e}'
            self._logger.info(message.format(epoch + 1, mvalid_mape2, mvalid_loss, mvalid_rmse, mvalid_mape, mvalidd_loss, mvalid_sfair, mvalid_dfair,\
                                             mvalid_be, mvalid_sa, mvalid_eq, mvalid_ben, mvalid_san, mvalid_eqn, \
    ...
    def evaluate(self, mode, sample_list, sample_map, sample_dict, district_nodes, yl_values,epoch,total_nodes):
        ...
        with torch.no_grad():
            for X, label in self._dataloader[mode + '_loader'].get_iterator():
                ...
                loss_message = 'Epoch: {:03d}, Batch_num:{:03d}, mape2:{:.4f}, Loss1: {:.4f}, Dis Loss: {:.4f}, Loss3: {:.4f}, Loss4: {:.4f}, Benefit: {:.4f}, Sacrifice: {:.4f}, Equal: {:.4f}, Be: {:.4f}, Sa: {:.4f}, Eq: {:.4f}'  # , Loss44: {:.4f}
                self._logger.info(loss_message.format(epoch + 1, batch_count+1, mape2, loss1, loss2, loss3, loss4, greater_than_zero, less_than_zero, equal_to_zero, ebenum, esanum,eeqnum)) # , loss44))
        ...
        if mode == 'val':
            ...
            , np.mean(eequal_list)/(np.mean(ebenefit_list)+np.mean(esarcifice_list)+np.mean(eequal_list))  # 一个epoch内！多个batch所以mean

            return mvalid_loss, mvalid_mape, mvalid_mape2, mvalid_rmse, mvalidd_loss, mvalid_sfair, mvalid_dfair , mvalid_be, mvalid_sa, mvalid_eq, mvalid_ben, mvalid_san, mvalid_eqn
            ...
            log = 'Average Test mape2: {:.4f}, MAE: {:.4f}, RMSE: {:.4f}, MAPE: {:.4f}, D_Loss: {:.4f}, SFair: {:.4f}, DFair: {:.4f},  Be {:.4f}, Sa {:.4f}, Eq {:.4f}, Bnum {:.4f}, Snum {:.4f},Enum {:.4f}.'
