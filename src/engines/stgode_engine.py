...
class STGODE_Engine(BaseEngine):
    ...
    def train(self, yl_values, sample_map, district_nodes):
        ...
        for epoch in range(self._max_epochs):
            t1 = time.time()
            mtrain_loss, mtrain_mape, mtrain_mape2, mtrain_rmse, mtrain_sfair, mtrain_dfair = self.train_batch(yl_values, sample_map, district_nodes, epoch)
            t2 = time.time()

            v1 = time.time()
            mvalid_loss, mvalid_mape, mvalid_mape2, mvalid_rmse, mvalid_sfair, mvalid_dfair = self.evaluate('val', yl_values, sample_map, district_nodes, epoch)
            v2 = time.time()
        ...
        # 将两个列表存储到文件中
        filename1 = 'ylvalue_s{}_HK_3.pkl'.format(self._seed)
        with open(os.path.join(self._save_path, filename1), 'wb') as f:
            pickle.dump((yl_list_train, yl_list_val), f)

        filename2 = 'ylvalue_s{}_HK_3_node.pkl'.format(self._seed)
    ...
    def evaluate(self, mode, yl_values, sample_map, district_nodes, epoch):
        ...
        if mode == 'val':
            ...
            log = 'Average Test mape2: {:.4f}, MAE: {:.4f}, Test RMSE: {:.4f}, Test MAPE: {:.4f}, Test SFair: {:.4f}, Test DFair: {:.4f}'
