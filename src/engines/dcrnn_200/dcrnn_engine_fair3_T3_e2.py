...
# 为了和阈值比较，基于每个节点计算相应的mape，输入为(b,t,n,c)，输出为(b,t,n,c)
def calcu_mape_data(preds, labels, null_val):
...
# 阈值比较，得到标签：yl_values阈值，大于表示误差大判断为l，小于表示误差小判断为y
def give_yl_label(mape_data, yl_values): 
...
class FSAMPE_Engine(BaseEngine):
    def __init__(self, criterion, d_lrate, model_params, D_params, optimizer_D, scheduler_D, a_loss3, a_loss4, \
                 T_dynamic, sample_num, district13_road_index, **args):
    ...
    def save_model(self, save_path):
        ...
        filename = 'final_model_s{}_HK200_2.pt'.format(self._seed)
    ...
    def train_batch(self, time_T, sample_list, sample_map, sample_dict, district_nodes, yl_values, epoch, total_nodes):
        ...
        for X, label in self._dataloader['train_loader'].get_iterator():
            ...
            mape_data = calcu_mape_data(pred, label, mask_value_single)
            yl_label = give_yl_label(mape_data, yl_values)
            ...
            # loss3 = static_cal3(new_pred, new_label, self._device) # 静态公平正则化
    ...
    def train(self, time_T, sample_list, sample_map, sample_dict, district_nodes, yl_values):
        ...
        filename = 'ylvalue_s{}_HK_3_1e-2_node_mape2_S_b48.pkl'.format(self._seed)
