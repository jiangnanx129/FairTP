...
class FSAMPE_Engine(BaseEngine):
    def __init__(self, district13_road_index, **args):  # criterion不用
    ...
    def save_model(self, save_path):
        ...
        filename = 'final_model_s{}_SDALL_200_5.pt'.format(self._seed)
    ...
    def train_batch(self, district_nodes, epoch):
        ...
        for X, label in self._dataloader['train_loader'].get_iterator():
            ...
            loss3 = static_cal(new_pred,new_label, self._device) # 静态公平正则化，follow当前文件最开始注释的函数
            # _,loss3 = static_cal(new_pred,new_label) # 静态公平正则化, follow IDF_baselines.py中的函数
            # loss3 = static_cal3(new_pred,new_label, self._device)
    ...
    def train(self, district_nodes):
        ...
        for epoch in range(self._max_epochs):
            ...
            message = 'Epoch: {:03d}, Train mape2: {:.4f}, Train Loss: {:.4f}, RMSE: {:.4f}, MAPE: {:.4f}, SFair: {:.4f}, DFair: {:.4f},  Valid mape2: {:.4f}, Loss: {:.4f}, RMSE: {:.4f}, MAPE: {:.4f}, SFair: {:.4f}, DFair: {:.4f}, Train Time: {:.4f}s/epoch, Valid Time: {:.4f}s, LR: {:.4e}'
        ...
        '''注释'''
        # 用这个！计算的是mape！
        # filename1 = 'ylvalue_s{}_HK_3_1e-2_node_mape2_S_b48_all.pkl'.format(self._seed)
