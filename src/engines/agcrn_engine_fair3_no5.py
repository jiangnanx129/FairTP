...
def static_cal2(pred,label): # (b, t, 13, 1), 13表示区域数量，1表示预测的交通值维度
    ...
    for i in range(pred.shape[1]-1): # 区域数量-1-->13-1=12 # i = 0123456789--11
        ...
        for j in range(i + 1, pred.shape[1]): 
            ...
            mpe_diff.append(torch.mean(torch.abs(mpe_i - mpe_j))) # 区域i和区域j，每个时刻误差的绝对值
            mape_diff.append(torch.mean(torch.abs(mape_i - mape_j))) # sum可换成mean
            mae_diff.append(torch.mean(torch.abs(mae_i - mae_j)))    
            '''sum换成mean'''
            idx += 1
...
class AGCRN_Engine(BaseEngine):
    ...
    def save_model(self, save_path):
        ...
        filename = 'final_model_s{}_HK3_disout7_1b_test.pt'.format(self._seed) # 1bb4
        torch.save(self.model.state_dict(), os.path.join(save_path, filename))
    
    def load_model(self, save_path):
        filename = 'final_model_s{}_HK3_disout7_1b_test.pt'.format(self._seed)
    ...
    def train_batch(self, sample_list, sample_map, sample_dict, district_nodes, yl_values, epoch, total_nodes):
        ...
        for X, label in self._dataloader['train_loader'].get_iterator():
            ...
            # loss3_mpe, loss3_mape, loss3 = static_cal2(new_pred,new_label) # 静态公平正则化 # 原1bb4
            ...
            # static_fair_mae.append(loss3_mae.item()) # mae
    ...
    def evaluate(self, mode, sample_list, sample_map, sample_dict, district_nodes, yl_values,epoch,total_nodes):
        ...
        with torch.no_grad():
            for X, label in self._dataloader[mode + '_loader'].get_iterator():
                ...
                mape = masked_mape(new_pred, new_label, mask_value).item()
                rmse = masked_rmse(new_pred, new_label, mask_value).item()
                # loss3_mpe, loss3_mape, loss3 = static_cal2(new_pred,new_label) # 静态公平正则化 # 原始1bb4
                ...
                loss_message = 'Epoch: {:03d}, Batch_num:{:03d}, mape2:{:.4f}, Loss1: {:.4f}, Dis Loss: {:.4f}, Loss3: {:.4f}, Loss4: {:.4f}, Benefit: {:.4f}, Sacrifice: {:.4f}, Equal: {:.4f}' # , region_mae:{}'  # , Loss44: {:.4f}
                self._logger.info(loss_message.format(epoch + 1, batch_count+1, mape2, loss1, loss2, loss3, loss4, greater_than_zero, less_than_zero, equal_to_zero)) # , , loss_region  loss44))
