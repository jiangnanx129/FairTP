...
class ASTGCN_Engine(BaseEngine):
    ...
    def save_model(self, save_path):
        ...
        filename = 'final_model_s{}_HK_2.pt'.format(self._seed)
    ...
    def train_batch(self, yl_values, sample_map, district_nodes, epoch):
        ...
        for X, label in self._dataloader['train_loader'].get_iterator():
            ...
            loss3_mpe, loss3_mape, loss3_mae = static_cal2(new_pred,new_label) # 静态公平正则化 # IDF 无make_values
    ...
    def train(self, yl_values, sample_map, district_nodes):
        ...
        for epoch in range(self._max_epochs):
            t1 = time.time()
            mtrain_loss, mtrain_mape, mtrain_mape2, mtrain_mpe2, mtrain_mae2, mtrain_rmse, mtrain_sfair, mtrain_dfair, mtrain_sfair_mae = self.train_batch(yl_values, sample_map, district_nodes, epoch)
            t2 = time.time()

            v1 = time.time()
            mvalid_loss, mvalid_mape, mvalid_mape2, mvalid_mpe2, mvalid_mae2, mvalid_rmse, mvalid_sfair, mvalid_dfair, mvalid_sfair_mae = self.evaluate('val', yl_values, sample_map, district_nodes, epoch)
            v2 = time.time()
