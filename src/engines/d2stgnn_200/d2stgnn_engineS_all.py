...
class D2STGNN_Engine(BaseEngine):
    def __init__(self, cl_step, warm_step, horizon, adj_mx, sam_num, T_dynamic, sample_num, district13_road_index, **args):
    ...
    def save_model(self, save_path):
        ...
        filename = 'final_model_s{}_HK_3_2e-3_S2_b24_all.pt'.format(self._seed)
    ...
    def train_batch(self, sample_list, time_T, yl_values, sample_map, district_nodes, epoch):
        ...
        for X, label in self._dataloader['train_loader'].get_iterator():
            ...
            pred = self.model(X, label) # 输出dis_out但在baseline中不用
            ...
            loss3_mape = static_cal(new_pred, new_label, self._device) # RSF静态公平正则化
            ...
            loss_message = 'Epoch: {:03d}, Batch_num:{:03d}, mape2:{:.4f}, Loss1: {:.4f}, Loss3_mape: {:.4f}, Loss4: {:.4f}'
            self._logger.info(loss_message.format(epoch + 1, batch_count+1, mape2, loss, loss3_mape, loss4))
    ...
    def train(self, sample_list, time_T, yl_values, sample_map, district_nodes):
        ...
        for epoch in range(self._max_epochs):
            t1 = time.time()
            mtrain_loss, mtrain_mape, mtrain_mape2, mtrain_mpe2, mtrain_mae2, mtrain_rmse, mtrain_sfair, mtrain_dfair = self.train_batch(sample_list, time_T, yl_values, sample_map, district_nodes, epoch)
            t2 = time.time()

            v1 = time.time()
            mvalid_loss, mvalid_mape, mvalid_mape2, mvalid_mpe2, mvalid_mae2, mvalid_rmse, mvalid_sfair, mvalid_dfair = self.evaluate('val', sample_list, time_T, yl_values, sample_map, district_nodes, epoch)
            v2 = time.time()
            ...
            message = 'Epoch: {:03d}, Train mape2: {:.4f}, Train mpe2: {:.4f}, Train mae2: {:.4f}, Train Loss: {:.4f}, Train RMSE: {:.4f}, Train MAPE: {:.4f}, Train SFair_mape: {:.4f}, Train DFair: {:.4f}, Valid mape2: {:.4f}, Valid mpe2: {:.4f}, Valid mae2: {:.4f}, Valid Loss: {:.4f}, Valid RMSE: {:.4f}, Valid MAPE: {:.4f}, Valid SFair: {:.4f}, Valid DFair: {:.4f}, Train Time: {:.4f}s/epoch, Valid Time: {:.4f}s, LR: {:.4e}'
            self._logger.info(message.format(epoch + 1, mtrain_mape2, mtrain_mpe2, mtrain_mae2, mtrain_loss, mtrain_rmse, mtrain_mape, mtrain_sfair, mtrain_dfair,\
                                             mvalid_mape2, mvalid_mpe2, mvalid_mae2, mvalid_loss, mvalid_rmse, mvalid_mape, mvalid_sfair, mvalid_dfair,\
                                             (t2 - t1), (v2 - v1), cur_lr))
        ...
        # 将两个列表存储到文件中
        # filename1 = 'ylvalue_s{}_HK_3_2e-3_node_mpe2_S2_b24_all.pkl'.format(self._seed)
        # with open(os.path.join(self._save_path, filename1), 'wb') as f:
        #     pickle.dump((yl_list_train, yl_list_val), f)

        # filename2 = 'ylvalue_s{}_HK_3_2e-3_node_mae2_S2_b24_all.pkl'.format(self._seed)
        # with open(os.path.join(self._save_path, filename2), 'wb') as f:
        #     pickle.dump((yl_list_train_node, yl_list_val_node), f)

        # # 用这个！计算的是mape！
        # filename1 = 'ylvalue_s{}_HK_3_2e-3_node_mape2_S2_b24_all.pkl'.format(self._seed)
        # with open(os.path.join(self._save_path, filename1), 'wb') as f:
    ...
    def evaluate(self, mode, sample_list, time_T, yl_values, sample_map, district_nodes, epoch):
        ...
        with torch.no_grad():
            for X, label in self._dataloader[mode + '_loader'].get_iterator():
                ...
                loss_message = 'Epoch: {:03d}, Batch_num:{:03d}, mape2: {:.4f}, mpe2: {:.4f}, mae2: {:.4f}, Loss1: {:.4f}, Loss3_mape: {:.4f}, Loss4: {:.4f}'
                self._logger.info(loss_message.format(epoch + 1, batch_count+1, mape2, mpe2, mae2, loss, loss3_mape, loss4))
        ...
        if mode == 'val':
            ...
            mae, mape, mape2, mpe2, mae2, rmse, mvalid_sfair, mvalid_dfair = np.mean(e_loss), np.mean(e_mape), np.mean(e_mape2), np.mean(e_mpe2), np.mean(e_mae2), np.mean(e_rmse), np.mean(estatic_fair), np.mean(edynamic_fair)
            return mae, mape, mape2, mpe2, mae2, rmse, mvalid_sfair, mvalid_dfair
            ...
            log = 'Average Test mape2: {:.4f}, mpe2: {:.4f}, mae2: {:.4f}, MAE: {:.4f}, Test RMSE: {:.4f}, Test MAPE: {:.4f}, Test SFair_mae: {:.4f}, Test DFair: {:.4f}'
            self._logger.info(log.format(np.mean(e_mape2), np.mean(e_mpe2), np.mean(e_mae2), np.mean(e_loss), np.mean(e_rmse), np.mean(e_mape), np.mean(estatic_fair), np.mean(edynamic_fair)))
