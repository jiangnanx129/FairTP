...
def masked_mae2(preds, labels, null_val):
    if torch.isnan(null_val):
        mask = ~torch.isnan(labels)
    else:
        mask = (labels != null_val)
...
def masked_mpe2(preds, labels, null_val):
    if torch.isnan(null_val):
        mask = ~torch.isnan(labels)
    else:
        mask = (labels != null_val)
...
def masked_mape33(preds, labels):
...
'''为计算动态，要保留梯度'''
def masked_mae3(preds, labels, null_val): # （b,t,13,c）
    if torch.isnan(null_val):
        mask = ~torch.isnan(labels)
    else:
        mask = (labels != null_val)
    ...
    loss = torch.mean(loss, dim=(0, 1, 3), keepdim=True)
    loss = loss.squeeze()

    return loss # shape为[13]

# 用的mean
def masked_mape3(preds, labels, null_val):
    if torch.isnan(null_val):
        mask = ~torch.isnan(labels)
    else:
        mask = (labels != null_val)
...
def masked_mape3_2(preds, labels, null_val):
    if torch.isnan(null_val):
        mask = ~torch.isnan(labels)
    else:
        mask = (labels != null_val)
    ...
    loss = torch.sum(loss, dim=(0, 1, 3), keepdim=True)
    loss = loss.squeeze()
