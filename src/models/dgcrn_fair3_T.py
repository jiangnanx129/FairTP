...
class DGCRN(BaseModel):
    ...
    '''
    def __init__(self, device, gcn_depth, rnn_size, hyperGNN_dim, node_dim, \
    ...
    def forward(self, input, predefined_adj, label=None, batches_seen=None, task_level=12):  # (b, t, n, f)
        ...
        for i in range(self.seq_len):
            # print(x.shape, x[..., i].shape,"======") # torch.Size([64, 1, 350, 12]) torch.Size([64, 1, 350])
            kkk = x[..., i]
            Hidden_State, Cell_State = self.step(kkk, # 原始，3去掉时间只有1维度，torch.squeeze(x[..., i])
                                                 Hidden_State, Cell_State,
                                                 predefined_adj, 'encoder', i)
            if outputs is None:
        ...
        for i in range(task_level):
            try:
                decoder_input = decoder_input # torch.cat([decoder_input, timeofday[..., i]], dim=1)
            except:
                print(decoder_input.shape) #, timeofday.shape)
                sys.exit(0)
            Hidden_State, Cell_State = self.step(decoder_input, Hidden_State,
                                                 Cell_State, predefined_adj,
                                                 'decoder', None)

            '''鉴别器！'''
            dis_out_1 = self.classify(Hidden_State)
            ...
            dis_out_final.append(dis_out_1)
        ...
        dis_out_final = torch.stack(dis_out_final, dim=1)

        outputs_final = outputs_final.view(batch_size, self.node_num,
                                           task_level, self.output_dim).transpose(1, 2)
        dis_out_final = dis_out_final.view(batch_size, self.node_num,
                                           task_level, self.output_dim).transpose(1, 2)
        # print("111111",outputs_final.shape, dis_out_final.shape)
        return outputs_final, dis_out_final
