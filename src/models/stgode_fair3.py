...
class STGODE(BaseModel):
    ...
    def __init__(self, **args):   # A_sp, A_se,  
        ...
        self.sp_blocks = nn.ModuleList(
            [nn.Sequential(
                ...
                node_num=self.node_num)) for _ in range(3)
            ])
    ...
    def forward(self, x, sp_adj, se_adj, label=None):  # (b, t, n, f)
        ...
        for blk in self.sp_blocks:
            outs.append(blk(x, sp_adj))
        # semantic graph
        for blk in self.se_blocks:
            outs.append(blk(x, se_adj))
        outs = torch.stack(outs)
        x = torch.max(outs, dim=0)[0]
        # x为(b,t,n,64)
        dis_out = self.classify(x) # 输入(b,t,n,64), 输出(b,t,n,1)
...
class ODEFunc(nn.Module):
    def __init__(self, feature_dim, temporal_dim):
        ...
        self.x0 = None
        self.alpha = nn.Parameter(0.8 * torch.ones(450)) # adj.shape[1]
        self.beta = 0.6
    ...
    def forward(self, t, x, new_adj):
        alpha = torch.sigmoid(self.alpha).unsqueeze(-1).unsqueeze(-1).unsqueeze(0)
        xa = torch.einsum('ij, kjlm->kilm', new_adj, x)
