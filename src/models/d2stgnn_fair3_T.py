...
class D2STGNN(BaseModel):
    ...
    def __init__(self, model_args, **args):
        ...
        self._in_feat = model_args['num_feat'] # 1
        self._hidden_dim = model_args['num_hidden'] # 32
        self._node_dim = model_args['node_hidden'] # 12
        self._forecast_dim = 256
        self._output_hidden = 512

        self._node_num = self.node_num
        self._k_s = model_args['k_s'] # 2
        self._k_t = model_args['k_t'] # 3
        self._num_layers = model_args['layer'] # 5
        ...
        self.out_fc_2 = nn.Linear(self._output_hidden, model_args['gap']) # 3

        self.classify = nn.Sequential(
            nn.Linear(self._output_hidden, model_args['gap']),
            # nn.ReLU(),
            nn.Sigmoid()
        )
    ...
    def _graph_constructor(self, pre_defined_graph, **inputs):
        ...
        if self._model_args['dy_graph']:
            dynamic_graph = self.dynamic_graph_constructor(pre_defined_graph, **inputs)
        else:
            dynamic_graph = []
    ...
    def _prepare_inputs(self, history_data):
        num_feat = self._model_args['num_feat'] # 1
    ...
    def forward(self, history_data, pre_defined_graph, label=None):  # (b, t, n, f)
        ...
        static_graph, dynamic_graph = self._graph_constructor(pre_defined_graph=pre_defined_graph, node_embedding_u=node_embedding_u, node_embedding_d=node_embedding_d, history_data=history_data) # , time_in_day_feat=time_in_day_feat, day_in_week_feat=day_in_week_feat)
        ...
        for _, layer in enumerate(self.layers):
            inh_backcast_seq_res, dif_forecast_hidden, inh_forecast_hidden = layer(history_data=inh_backcast_seq_res, dynamic_graph=dynamic_graph, \
                                                                                   static_graph=static_graph, node_embedding_u=node_embedding_u, \
                                                                                    node_embedding_d=node_embedding_d, pre_defined_graph=pre_defined_graph) # , time_in_day_feat, day_in_week_feat)
        ...
        # print(forecast_hidden.shape) # torch.Size([36, 4, 450, 256])

        forecast = self.out_fc_2(F.relu(self.out_fc_1(F.relu(forecast_hidden))))
        # print(forecast.shape) # [36,4,450,3]

        dis_out = self.classify(F.relu(self.out_fc_1(F.relu(forecast_hidden)))
                                )
        ...
        dis_out = dis_out.transpose(1,2).contiguous().view(dis_out.shape[0], dis_out.shape[2], -1)
    
        return forecast.transpose(1, 2).unsqueeze(-1), dis_out.transpose(1, 2).unsqueeze(-1)
...
class DecoupleLayer(nn.Module):
    ...
    def forward(self, history_data, dynamic_graph, static_graph, node_embedding_u, node_embedding_d, pre_defined_graph): # , time_in_day_feat, day_in_week_feat):
        gated_history_data = self.estimation_gate(node_embedding_u, node_embedding_d, history_data) # , time_in_day_feat, day_in_week_feat

        dif_backcast_seq_res, dif_forecast_hidden = self.dif_layer(history_data=history_data, gated_history_data=gated_history_data, dynamic_graph=dynamic_graph, static_graph=static_graph, pre_defined_graph=pre_defined_graph)
...
class DifBlock(nn.Module):
    def __init__(self, hidden_dim, forecast_hidden_dim=256, use_pre=None, dy_graph=None, sta_graph=None, **model_args):
        ...
        self.localized_st_conv = STLocalizedConv(hidden_dim, use_pre=use_pre, dy_graph=dy_graph, sta_graph=sta_graph, **model_args)
    ...
    def forward(self, history_data, gated_history_data, dynamic_graph, static_graph, pre_defined_graph):

        hidden_states_dif = self.localized_st_conv(gated_history_data, dynamic_graph, static_graph, pre_defined_graph)

        forecast_hidden = self.forecast_branch(gated_history_data, hidden_states_dif, self.localized_st_conv, dynamic_graph, static_graph,pre_defined_graph)
...
class STLocalizedConv(nn.Module):
    def __init__(self, hidden_dim, use_pre=None, dy_graph=None, sta_graph=None, **model_args):
        super().__init__()
        ...
        self.support_len = 2 + int(dy_graph) + int(sta_graph)
        self.num_matric = (int(use_pre) * 2 + 2 * int(dy_graph) + int(sta_graph)) * self.k_s + 1
    ...
    def forward(self, X, dynamic_graph, static_graph, pre_defined_graph):
        ...
        if self.use_predefined_graph:
            support = support + self.get_graph(pre_defined_graph)
...
class DifForecast(nn.Module):
    ...
    def forward(self, gated_history_data, hidden_states_dif, localized_st_conv, dynamic_graph, static_graph,pre_defined_graph):
        predict = []
        ...
        for _ in range(int(self.output_seq_len / self.model_args['gap'])-1):
            ...
            predict.append(localized_st_conv(_1, dynamic_graph, static_graph, pre_defined_graph))
...
class DynamicGraphConstructor(nn.Module):
    def __init__(self, **model_args):
        ...
        self.mask = Mask(**model_args) # 11111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111
    ...
    def forward(self, pre_defined_graph, **inputs):
        ...
        dist_mx = self.mask(dist_mx, pre_defined_graph) # # 11111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111
...
class Mask(nn.Module):
    def __init__(self, **model_args):
        super().__init__()
        self.mask = None # model_args['adjs']


    def _mask(self, index, adj, pre_defined_graph):
        mask = pre_defined_graph[index] + torch.ones_like(pre_defined_graph[index]) * 1e-7
        return mask.to(adj.device) * adj


    def forward(self, adj, pre_defined_graph):
        result = []
        for index, _ in enumerate(adj):
            result.append(self._mask(index, _, pre_defined_graph))
        return result
