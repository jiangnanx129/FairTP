...
'''
1. dcrnn, 学到表示1
2. 对抗学习，输入表示1，label，输出表示2
3. 表示2用于计算任务损失，静态损失（每个batch一个staticf）
4. 增加退火算法自适应采样！(follow：动态+静态损失)
'''
...
class FSAMPLE(BaseModel):
    ...
    def forward(self, source, target, sample_list, yl_values, iter=None):  # (b, t, n, f)
        ...
        x = self.classif(dcgru4_state_tensor.reshape(t,b,n,-1)) # 返回的x为(t*b*n,co) 
        dis_labels = torch.where(x > 0.5, torch.ones_like(x), torch.full_like(x, -1)).reshape(b,t,n,-1) # 鉴别器输出的标签(t*b*n,co),为-1/1

       
        return o, dis_labels, yl_label, x, mape_data
