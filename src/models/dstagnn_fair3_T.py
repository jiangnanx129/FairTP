...
class DSTAGNN(BaseModel):
    ...
    '''
    def __init__(self, device, order, nb_block, nb_chev_filter, nb_time_filter, \
                 time_stride, d_model, d_k, d_v, n_head, **args):
        ...
        self.BlockList = nn.ModuleList([DSTAGNN_block(device, self.input_dim, self.input_dim, order,
                                                     ...
                                                     self.node_num, self.seq_len, d_model, d_k, d_v, n_head)])
        ...
        self.BlockList.extend([DSTAGNN_block(device, self.input_dim * nb_time_filter, nb_chev_filter, order,
                                            ...
                                            self.node_num, self.seq_len // time_stride, d_model, d_k, d_v, n_head) for _ in range(nb_block - 1)])
    ...
    def forward(self, x, cheb_polynomials, adj_pa, label=None):  # (b, t, n, f)
        x = x.permute(0, 2, 3, 1)  # (b, n, f, t)

        need_concat = []
        res_att = 0
        for block in self.BlockList:
            x, res_att = block(x, res_att, cheb_polynomials, adj_pa)
            need_concat.append(x)

        final_x = torch.cat(need_concat, dim=-1)
        output = self.final_conv(final_x.permute(0, 3, 1, 2))[:, :, :, -1].permute(0, 2, 1)

        dis_out = self.classify(output)
        output = self.final_fc(output)

        output = output.unsqueeze(-1).transpose(1, 2)
        dis_out = dis_out.unsqueeze(-1).transpose(1, 2)
...
class DSTAGNN_block(nn.Module):
    def __init__(self, device, num_of_d, in_channels, K, nb_chev_filter, nb_time_filter, time_stride,
                  node_num, seq_len, d_model, d_k, d_v, n_head):
        ...
        self.cheb_conv_SAt = cheb_conv_withSAt(K, in_channels, nb_chev_filter, node_num, device)
    ...
    '''新家邻接矩阵adj_pa'''
    def forward(self, x, res_att, cheb_polynomials, adj_pa): 
        ...
        spatial_gcn = self.cheb_conv_SAt(x, STAt, cheb_polynomials, adj_pa)
...
class cheb_conv_withSAt(nn.Module):
    def __init__(self, K, in_channels, out_channels, node_num, device):
    ...
    def forward(self, x, spatial_attention, cheb_polynomials, adj_pa):
