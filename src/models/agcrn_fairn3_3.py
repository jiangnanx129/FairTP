...
class AGCRN(BaseModel):
    ...
    def __init__(self, embed_dim, rnn_unit, num_layer, cheb_k, **args):
        ...
        # self.classif = BinaryClassifier(rnn_unit, self.output_dim, self.horizon)

        self.classif = BinaryClassifier2(rnn_unit, self.output_dim, self.horizon)
    ...
    def forward(self, source, label=None):  # (b, t, n, f)
        ...
        output1 = output[:, -1:, :, :] # (b, 1, N, rnn_unit)


        pred = self.end_conv(output1)
        # 输入(b,12,n,c), 输出(b,12,450,1)
        dis_out = self.classif(output.clone()) # 鉴别器 .clone()

        return pred, dis_out

'''参考fsample3.py'''
class BinaryClassifier(nn.Module):
    def __init__(self, input_dim, dis_output_dim, horizon):
        ...
        # self.final_conv = nn.Conv2d(1, self.horizon * self.dis_output_dim, kernel_size=(1, input_dim)) 
        self.sigmoid = nn.Sigmoid()  # Sigmoid 激活函数,将值映射到0-1

    def forward(self, x): # x为(b,t,n,ch)
        b,t,n,ch = x.shape
        # x = self.final_conv(x) # 输出(b,t,n,1)
        x = self.fc(x.reshape(-1,ch)) 
...
class BinaryClassifier2(nn.Module):
    def __init__(self, input_dim, dis_output_dim, horizon):
        ...
        '''linear或者Conv2d'''
        # self.fc = nn.Linear(input_dim, dis_output_dim)  # 全连接层
        self.final_conv = nn.Conv2d(12, 12, kernel_size=(1, input_dim)) 
        # 这个卷积层的作用是在输入的特征图上应用12个不同的1xinput_dim的卷积核，并生成12个输出特征图。每个输出特征图的大小由输入特征图的大小、卷积核的大小以及填充和步长等参数决定。
        # 12表示输入的通道数，也就是输入特征图的深度为12。
        # 12表示输出的通道数，也就是输出特征图的深度为12。
        self.sigmoid = nn.Sigmoid()  # Sigmoid 激活函数,将值映射到0-1
...
class AVWDCRNN(nn.Module):
    ...
    def forward(self, x, init_state, node_embed):
        ...
        current_inputs = x
        output_hidden = []
        for i in range(self.num_layer): # num_layer=2
            state = init_state[i]
            inner_states = []
            for t in range(seq_length):
                state = self.dcrnn_cells[i](current_inputs[:, t, :, :], state, node_embed)
                inner_states.append(state) # state (b,n,c)
