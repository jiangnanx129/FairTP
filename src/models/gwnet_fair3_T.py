...
class GWNET(BaseModel):
    ...
    def __init__(self, adp_adj, dropout, residual_channels, dilation_channels, \
        ...
        self.supports_len = 2 #len(supports)
        ...
        if adp_adj:
            ...
            self.supports_len += 1
        print('check supports length', "2" , self.supports_len) # len(supports)
    ...
    '''supports 由sample_list确定的采样矩阵决定'''
    def forward(self, input, supports, label=None):  # (b, t, n, f)
