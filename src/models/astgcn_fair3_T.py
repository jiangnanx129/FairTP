...
class ASTGCN(BaseModel):
    ...
    def __init__(self, device, order, nb_block, nb_chev_filter, nb_time_filter, time_stride, **args):
        ...
        self.BlockList = nn.ModuleList([ASTGCN_block(device, self.input_dim, order, nb_chev_filter, nb_time_filter, time_stride, self.node_num, self.seq_len)])
        self.BlockList.extend([ASTGCN_block(device, nb_time_filter, order, nb_chev_filter, nb_time_filter, 1, self.node_num, self.seq_len // time_stride) for _ in range(nb_block - 1)])
    ...
    def forward(self, x, adj_list, label=None):  # (b, t, n, f)
...
class ASTGCN_block(nn.Module):
    def __init__(self, device, in_channels, order, nb_chev_filter, nb_time_filter, time_strides, node_num, seq_len):
        ...
        self.cheb_conv_SAt = cheb_conv_withSAt(order, device, in_channels, nb_chev_filter)
    ...
    def forward(self, x, adj_list):
        ...
        spatial_gcn = self.cheb_conv_SAt(x, spatial_At, adj_list)
...
class cheb_conv_withSAt(nn.Module):
    def __init__(self, order, device, in_channels, out_channels):
    ...
    def forward(self, x, spatial_attention, adj_list):
