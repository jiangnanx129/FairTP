...
'''
区别于fsample3，定义两个模型：FSAMPLE和BinaryClassifier
1. encoder和decoder在FSAMPLE中
2. 分类器discriminator在BinaryClassifier中，独立开来
'''
...
class FSAMPLE(BaseModel):
    ...
    def forward(self, source, target, sample_list, yl_values, iter=None):  # (b, t, n, f)
        ...
        return o, dcgru4_state_tensor # 返回的结果未归一化
        '''
...
class BinaryClassifier(nn.Module):
    def __init__(self, device, input_dim, dis_output_dim):
        ...
        self.device = device
        self.input_dim = input_dim # 实质为hidden_size, 包含学到的时空依赖
        self.dis_output_dim = dis_output_dim
