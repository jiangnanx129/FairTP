...
class AGCRN(BaseModel):
    ...
    def forward(self, source, label=None):  # (b, t, n, f)
        ...
        output = output[:, -1:, :, :] # 取最后一个元素
        pred = self.end_conv(output) # 用最后一层卷积，1变为12T

        dis_out = self.classif(output) # 鉴别器 .clone()
...
class BinaryClassifier(nn.Module):
    def __init__(self, input_dim, dis_output_dim, horizon):
        ...
        self.final_conv = nn.Conv2d(1, self.horizon * self.dis_output_dim, kernel_size=(1, input_dim), bias=True) 
