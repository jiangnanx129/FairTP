...
class D2STGNN(BaseModel):
    ...
    def _graph_constructor(self, new_adj, **inputs):
        ...
        if self._model_args['dy_graph']:
            dynamic_graph = self.dynamic_graph_constructor(new_adj, **inputs)
        else:
            dynamic_graph = []
    ...
    def forward(self, new_adj, history_data, label=None):  # (b, t, n, f)
        ...
        static_graph, dynamic_graph = self._graph_constructor(new_adj = new_adj, node_embedding_u=node_embedding_u, node_embedding_d=node_embedding_d, history_data=history_data) # , time_in_day_feat=time_in_day_feat, day_in_week_feat=day_in_week_feat)
...
class DynamicGraphConstructor(nn.Module):
    ...
    def forward(self, new_adj, **inputs):
        ...
        dist_mx = self.mask(dist_mx, new_adj) # # 11111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111
...
class Mask(nn.Module):
    ...
    def _mask(self, index, adj, new_adj): # new_adj是会变化的，随着采样
        mask = new_adj[index] + torch.ones_like(new_adj[index]) * 1e-7
        return mask.to(adj.device) * adj


    def forward(self, adj, new_adj):
        result = []
        for index, _ in enumerate(adj):
            result.append(self._mask(index, _, new_adj))
        return result
