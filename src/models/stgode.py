...
class STGODE(BaseModel):
    ...
    def forward(self, x, label=None):  # (b, t, n, f)
        ...
        for blk in self.se_blocks:
            outs.append(blk(x))
        # print("1:", len(outs), outs[0].shape) # 1: 6 torch.Size([b, 450, 12, 64]) 
        outs = torch.stack(outs)
        # print("2:", outs.shape) # 2: torch.Size([6, b, 450, 12, 64])
        x = torch.max(outs, dim=0)[0]
        # print("3:", x.shape) # 3: torch.Size([b, 450, 12, 64])
        x = x.reshape((x.shape[0], x.shape[1], -1)) 
        # print("4:", x.shape) # 4: torch.Size([b, 450, 768])
        x = self.pred(x)
        # print("5:", x.shape) # 5: torch.Size([b, 450, 12])
        x = x.unsqueeze(-1).transpose(1, 2)
        # print("6:", x.shape) # 6: torch.Size([b, 12, 450, 1])
