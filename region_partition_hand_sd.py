...
def get_lat_lon(data_path):
    ...
    ca_meta = pd.read_csv(data_path+"sd_meta.csv")
    ...
    center_lat, center_lon = np.mean(lat), np.mean(lon)
    
    print("sd的区域范围：", max_lat, min_lat, max_lon, min_lon, center_lat, center_lon)
    # 35.602653 33.404943 -114.559672 -119.434005
    return lat_lon, max_lat, min_lat, max_lon, min_lon, center_lat, center_lon


def partition(lat_lon, max_lon, min_lon, max_lat, min_lat):
    # 计算每个区域的纬度和经度间隔
    latitude_interval = 0.2 #  (max_lat - min_lat) / x1
    longitude_interval = 0.2 # (max_lon - min_lon) / x1
    ...
    new_district_dict = {str(i): district_dict[key] for i, key in enumerate(district_dict)} # 


    print("sd区域分布+各区域节点数目：", district_numpy, np.sum(district_numpy)) 
    # print("节点分布字典：", district_dict)
    # print("区域中节点数目字典：", district_counts)
    # print("区域位置字典：", district_geo)
    with open(data_path+"sd_district.json", 'wb') as file:
        pickle.dump(district_dict, file)
    with open(data_path+"sd_district2.json", 'wb') as file:
        pickle.dump(new_district_dict, file)
    with open(data_path+"sd_district_counts.json", 'wb') as file:
        pickle.dump(district_counts, file)
    with open(data_path+"sd_district_geo.json", 'wb') as file:
...
def plot_roadcount_statistics(hb_regions_counts): # data是字典
    ...
    # 调整图像布局
    plt.tight_layout()
    plt.savefig("Barchart_13_count_sd.png", dpi=300)
    # 显示图像
    plt.show()


def draw_folium(lat_lon, district_dict, district_geo, center_lat, center_lon):
    colorlist = ['#4d30fc', '#dffc23', '#7c0750', '#d48707', '#df248a', '#5cd15d', '#e322d4', '#fa348e', '#4cddd7', '#273fa3', '#873a1a', '#750818', '#c1a0e5']

    m = folium.Map(location=[center_lat, center_lon],  tiles= 'http://thematic.geoq.cn/arcgis/rest/services/StreetThematicMaps/Gray_OnlySymbol/MapServer/tile/{z}/{y}/{x}',attr= '高德-英文对照',zoom_start=12) #, tiles=None)
    # for v, (key, values) in enumerate(district_geo.items()):
    #     lat1,lat2,lon1,lon2 = district_geo[key][0], district_geo[key][1], district_geo[key][2], district_geo[key][3]
    #     folium.Rectangle(bounds=[(lat1,lat2),(lon1,lon2)], color=colorlist[v], fill=True, fill_color=colorlist[v], fill_opacity=0.2).add_to(m)
    for v1, (key1, values1) in enumerate(district_dict.items()):
        for i in values1: # 节点index
            folium.CircleMarker(location=lat_lon[i], radius=3,color=colorlist[v1],fill=True,fill_color=colorlist[v1],fill_opacity=0.7).add_to(m)
            
    m.save("map716_region12.html")

'''
1. sd: 0.8624419999999944 0.7108570000000043
2. 716节点！
'''
data_path = './data/sd/'
lat_lon, max_lat, min_lat, max_lon, min_lon, center_lat, center_lon = get_lat_lon(data_path)
# 中心32.8574131424581 -117.13375297067039
partition(lat_lon, max_lon, min_lon, max_lat, min_lat)
print(max_lat-min_lat, max_lon-min_lon)

# 画图
with open(data_path+"sd_district_counts.json", 'rb') as file:
    hb_regions_counts = pickle.load(file)
plot_roadcount_statistics(hb_regions_counts)

with open(data_path+"sd_district.json", 'rb') as file:
    district_dict = pickle.load(file)
with open(data_path+"sd_district_geo.json", 'rb') as file:
    district_geo = pickle.load(file)

draw_folium(lat_lon, district_dict, district_geo, center_lat, center_lon)
